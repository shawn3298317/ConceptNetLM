...
class SemFeature():
	def __init__(self):
		...
		self.docs = np.load("../../corpus/user/train.npy")[8400:16800]
		self.target = ["n", "v", "adj"]
		self.depth_l = 6
	...
	def createConceptPair(self, corpus, target_corpus):
		...
		for sentence, target_sentence in zip(corpus,target_corpus):
			...
			for i in range(len(sentence)):
				...
				#curr_word = lemma(sentence[i]).encode('utf-8')
				#sentence_conceptPair.append([ [curr_word, next_word], [prev_word, curr_word] ])
...
def main():
	...
	np.save(open("../../data/feature/sem/user_train_2.npy" % user, "wb"), feat)########
