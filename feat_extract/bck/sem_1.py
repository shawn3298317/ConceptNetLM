...
class SemFeature():
	def __init__(self):
		...
		self.docs = dict()
		self.docs["background_1"] = np.load("../../corpus/background_200000.npy")[:100000]
		self.docs["train"] = np.load("../../corpus/user/train.npy")
		#self.docs["valid"] = np.load("../../corpus/user/valid.npy")
		#self.docs["test"]  = np.load("../../corpus/user/test.npy")
		self.target = ["n", "v", "adj"]
	...
	def createConceptPair(self, filename, target_corpus):
		corpus_conceptPair = []
		for sentence, target_sentence in zip(self.docs[filename],target_corpus):
	...
	def Semantic(self, filename):
		...
		pair = self.createConceptPair(filename, target_corpus)
