...
print "Training %s with %s feature" % (args.data,args.feat)
...
print "Loading data...."
#X_train = np.asarray(list(np.load("../data/X_train.npy")) + list(np.load("../data/user/X_train.npy")))
X_valid = np.load("../data/user/X_valid.npy")
#Y_train = np.asarray(list(np.load("../data/Y_train.npy")) + list(np.load("../data/user/Y_train.npy")))
Y_valid = np.load("../data/user/Y_valid.npy")
feat_name = ""
if args.feat == 'pos':
	feat_name = 'pos'
...
for ep in range(EPOCH):
	...
	for t in range(DATA_LENGTH/BATCH):
		...
		time_st = time.time()
		new_X, new_Y, new_F, length = loadBatch(X_train, Y_train, F_train, t, MAX_LEN, BATCH, N, FEAT_SIZE, args.feat)
		time_md2 = time.time()
		if args.feat != None:	
			loss1 = model.train_on_batch([new_X,new_F], new_Y, class_weight=None, sample_weight=None)
		else:
			loss1 = model.train_on_batch(new_X, new_Y, class_weight=None, sample_weight=None)
		total_loss1 += loss1
		total_acc1 += 0
