...
def Parse():
	...
	with open('user_text.txt','r') as f:
		...
		for line in f:
			...
			#print toAdd
			#print ' '.join(toAdd)
			if len(toAdd) >= 1 and toAdd not in Corpus[user] and "" not in toAdd:
	...
	for user in Corpus:
		...
		Corpus[user] = Corpus[user]
		#if len(Corpus[user]) < 1000:
		#	print "user", user, "contain", len(Corpus[user]), "sentences' len from", len(Corpus[user][0]), "to", len(Corpus[user][-1])
	#print Corpus["polascheps"]
	#np.save('background_corpus.npy',Corpus)
	#for user in Corpus:
	np.save(open("../user_corpus_all.npy", "wb"), Corpus)
